...
namespace SupervisorMobility.Client.Services.JobObservationService
{
    public class JobObservationService : IJobObservationService
    {
        ...
        public async Task<List<JobObservation>> GetAllJobObservations(bool includeTree = false, bool includePeople = false, bool includeLup = false, bool includeHistory = false, bool includeCkAnswers = false, bool ForSosProgram = false, int year = 0, int userId = 0)
        {
            var response = await _http.GetAsync($"jobobservations?includeTree={includeTree}&includePeople={includePeople}&includeLup={includeLup}&includeHistory={includeHistory}&includeCkAnswers={includeCkAnswers}&ForSosProgram={ForSosProgram}&year={year}&userId={userId}");
