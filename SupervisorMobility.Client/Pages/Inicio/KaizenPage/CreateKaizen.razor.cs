...
namespace SupervisorMobility.Client.Pages.Inicio.KaizenPage
{
    public partial class CreateKaizen
    {
        ...
        string costText = "Ahorro de $133.47 por hora por reparaci�n de paneles con marca, se reparan 4 paneles por hora, total de paneles 900.  Total de horas = (900 / 4) = 225 horas   Total de ahorro = (225 * 133.47) = $30.030.75";
        string laborText = "Costo por modificaci�n de troquel $133.47 por hora, se usaron 18 horas, total: $2,402.46";
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender && !_cameraInitialized)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine("Permiso de acceso a la c�mara no otorgado.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error al recargar la c�mara: {ex.Message}");
                }
            }
